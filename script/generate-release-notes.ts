...
export default class GenerateReleaseNotes {
    ...
    private expectedArgs = [
        ...
        },
        {
            key: 1,
            name: 'tagName',
            description: 'name of the GitHub tag that we use to generate the changelog'
        },
        {
            key: 2,
            name: 'githubToken',
    ...
    private expectedArgsString = this.expectedArgs.map(arg => `\${${arg.name}}`).join(' ');
    ...
    constructor() {
        ...
        for (const arg of this.expectedArgs) {
            if (!this.args[arg.key]) {
                console.error(`🔴 Missing CLI argument \${${arg.name}} (${arg.description}). Please run the script as follows: node -r ts-node/register script/generate-release-notes.ts ${this.expectedArgsString}`);
                process.exit(1);
            }
        }
    ...
    async run() {
        ...
        if (this.SUCCESSFUL_RELEASE_FILE_COUNT !== countFiles) {
            console.error(`🔴 Artifacts folder has ${countFiles} assets, expecting ${this.SUCCESSFUL_RELEASE_FILE_COUNT}. Please check the GH Actions artifacts to see which are missing.`);
            process.exit(1);
        }

        const releaseEntries = await this.generateReleaseNotesEntries();
        const draftReleaseNotes = this.generateDraftReleaseNotes(releaseEntries, shaEntries);
        const releaseNotesPath = __dirname + '/release_notes.txt';

        fs.writeFileSync(releaseNotesPath, draftReleaseNotes, {encoding: 'utf8'});

        console.log(`✅ All done! The release notes have been written to ${releaseNotesPath}`)
    }
    ...
    async generateReleaseNotesEntries(): Promise<Array<string>> {
        ...
        console.log(`Comparing commits between ${latestReleaseTag} and ${this.tagName}...`);
